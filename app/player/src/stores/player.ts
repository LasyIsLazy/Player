...
export const usePlayerStore = defineStore("player", () => {
  ...
  const prepare = async (songId: string, collectionId?: number) => {
    ...
    collectionId = collection.value.id;
    const songInfo = getSongInfo(songId, collectionId);
    if (!songInfo) {
      throw new Error(
    ...
    const listenProgress = () => {
      curTime.value = sound.value?.seek() ?? 0;
      duration.value = sound.value?.duration() ?? 0;
      if (status.value !== PlayStatus.Playing) {
        sound.value?.once("play", listenProgress);
        return;
      }
  ...
  const seek = (progress: number) => {
    ...
    sound.value.seek(duration * progress);
  };
  ...
  const getRandomSong = () => {
    if (!collection.value?.list.length) {
      return;
    }
    const song =
      collection.value.list[
        Math.floor(Math.random() * collection.value.list.length)
      ];
    return song;
  };
